...
log4js.addAppender(log4js.appenders.file('../logs/priority/priority_gateway_'+moment().format('YYYYMMDD')+'.log'), 'DESC');
...
var test = connection.query('SELECT * FROM queue WHERE delivery = 2 AND sms_type = 3 LIMIT ' + process.argv[2], function(err, rows, fields) {
    if (!err)
    {
        if(rows.length > 0){
